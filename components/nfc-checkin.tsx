...
export function NfcCheckin({ token }: { token: string }) {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const response = await fetch('/api/nfc/check-in', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ token }), cache: 'no-store' })
  ...
  return <div className="stack" style={{textAlign:'center'}}>
    ...
    <div className="row" style={{justifyContent:'center',gap:10,flexWrap:'wrap'}}><a className="btn btn-primary" href="/student/materials">ดูสื่อการสอน</a><a className="btn btn-secondary" href="/student/lab">เล่น 3D + มินิเกม</a></div>
    <p className="muted">แม้ครูยังไม่เปิดรอบเช็กชื่อ นักเรียนที่ล็อกอินแล้วก็ยังเปิดสื่อและบทเรียน 3D ได้</p>
  </div>
}
